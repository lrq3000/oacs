...
try:
    from runsnakerun import runsnake # runsnakerun needs wxPython lib, if it's not available then we pass. RunSnakeRun is only used for GUI visualisation, not for profiling (and you can still use pstats for console browsing)
except:
    pass
...
def runprofile(mainfunction, output, timeout = 0, calibrate=False):
    ...
    if calibrate:
        ...
        print('Calibration found value : %s' % cval)
